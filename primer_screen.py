...
rb = RestrictionBatch([BtsI, BspQI, NdeI, KpnI, EcoRI, MlyI])
...
for primer in asmFseqs:
    ...
    if len(seqsearch[BtsI])!=1 or len(seqsearch[BspQI])!=0 or\
        len(seqsearch[NdeI])!= 1 or len(seqsearch[KpnI])!=0 or\
        len(seqsearch[EcoRI])!=0 or len(seqsearch[MlyI])!=0:
...
for primer in asmRseqs:
    ...
    seqsearch = rb.search(current_seq)
    if len(seqsearch[BtsI])!=1 or len(seqsearch[BspQI])!=0 or\
        len(seqsearch[NdeI])!= 0 or len(seqsearch[KpnI])!=1 or\
        len(seqsearch[EcoRI])!=0 or len(seqsearch[MlyI])!=0:
...
for primer in ampFseqs:
    ...
    #find illegal sites
    seqsearch = rb.search(current_seq)
    if len(seqsearch[BtsI])!=0 or len(seqsearch[BspQI])!=1 or\
        len(seqsearch[NdeI])!= 0 or len(seqsearch[KpnI])!=0 or\
        len(seqsearch[EcoRI])!=0 or len(seqsearch[MlyI])!=0:
...
for primer in ampRseqs:
    #build the sequence
    current_seq = Seq('AA') + BtsaI_seq.reverse_complement() + primer.seq.reverse_complement()
